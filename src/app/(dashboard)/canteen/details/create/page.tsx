...
export default function Page() {
    ...
    const onSubmit = (data: any) => {
        if (facilities.length === 0) {
            toastMessage(`Please add some "Facilities"`, "e");
            return
        }
        setIsLoading(true)
        const formData = { ...data, facilities: facilities }
        axiosInstance.post("/api/canteen/facilities/create", formData)
            .then((res) => {
                setIsLoading(false)
                toastMessage(res.data.message, "s")
                router.push('/canteen/details');
            }).catch(() => {
                setIsLoading(false)
            })
    }
    ...
    const removeItem = (indexToRemove: number) => {
        ...
        let newArray = _facilities.filter((_, index) => index !== indexToRemove);
    ...
    return (
        <>
            ...
            <Box sx={{ display: "flex", justifyContent: "flex-start", mb: 1.5 }}>
                <Button
                    LinkComponent={Link}
                    href="/canteen/details"
                    variant="outlined" size="small" startIcon={<ArrowBackIosNewOutlinedIcon />}
                >Back</Button>
            </Box>
            <Paper sx={{ p: 2, height: "100%" }} elevation={0}>
                <Typography variant='h6' sx={{ color: "primary.main", mb: 2 }}>Add canteen facilities</Typography>
