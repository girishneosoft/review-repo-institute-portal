...
export default function Page() {
    ...
    const callApi = () => {
        ...
        axiosInstance.get("/api/culture/team_member/list").then((res) => {
    ...
    const onArchive = (row: any) => {
        showAlert({
            onAction: (status: string) => {
                if (status === "ok") {
                    axiosInstance.delete(`/api/culture/team_member/${row.id}`, { withCredentials: true }).then((res) => {
                        callApi();
                    })
                }
            }
        ...
        <>
            ...
            <Paper sx={{ padding: 2 }} elevation={0} >
                <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
                    ...
                    <Button LinkComponent={Link} href="/culture/team-members/create" variant="contained" size="small">Add Member</Button>
                </Box>
                <MuiDataGrid
                    density="compact"
